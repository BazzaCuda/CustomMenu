...
var
  ...
  vBlue:  byte;
begin
  vRed    := getRValue(colorToRGB(aColor));
  vGreen  := getGValue(colorToRGB(aColor));
  vBlue   := getBValue(colorToRGB(aColor));
  result  := format('$%.2X%.2X%.2X', [vRed, vGreen, vBlue]);
end;

function chooseColor(const aInitialColor: TColor): integer;
begin
  result := aInitialColor;
  var vColorDialog: TColorDialog := TColorDialog.Create(NIL);
  try
    vColorDialog.color    := aInitialColor;
    vColorDialog.options  := vColorDialog.options + [cdFullOpen, cdSolidColor];
    var vHWND := vColorDialog.handle;
    case vColorDialog.execute of TRUE: result := vColorDialog.color; end;
  finally
    vColorDialog.free;
  end;
end;
...
function TConfigForm.setColors: boolean;
begin
  color                                 := FBackgroundColor;
  checkboxPanel.color                   := FBackgroundColor;
  backPanel.color                       := FBackgroundColor;
  vst.color                             := FBackgroundColor;
  ...
  lblDragAndDropCommandFile.font.color  := FInfoColor;
  lblDragDropFolder.font.color          := FInfoColor;
  lblBrowseCommand.font.color           := FInfoColor;
  lblBrowseDirectory.font.color         := FInfoColor;
  lblCommandCategories.font.color       := FInfoColor;
  editSubMenuName.EditLabel.font.color  := FInfoColor;
  lblisSubMenu.font.color               := FInfoColor;
  lblHelp.font.color                    := FInfoColor;
  lblIconGroup.font.color               := FInfoColor;
  lblFeedback.font.color                := FInfoColor;
  lblAutoFill.font.color                := FInfoColor;
  lblWriteRegistry.font.color           := FInfoColor;
  lblResizeTheWindow.font.color         := FInfoColor;
  lblBackgroundColor.font.color         := FInfoColor;
